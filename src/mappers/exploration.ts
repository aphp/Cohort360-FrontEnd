...
import { DocumentStatuses, Order } from 'types/searchCriterias'
import { CohortComposition, CohortPMSI, CohortResults } from 'types'
...
export const mapDocumentsToColumns = (deidentified: boolean): Column[] => {
  return [
    { label: 'Statut' },
    { label: 'Nom / Date', code: Order.DATE },
    { label: `IPP${deidentified ? ' chiffré' : ''}`, code: Order.SUBJECT_IDENTIFIER },
    { label: `NDA${deidentified ? ' chiffré' : ''}` },
    { label: 'Unité exécutrice' },
  ...
  list.forEach((elem) => {
    ...
    const row: Row = [
      ...
      },
      {
        ...
        value: `${elem.description ?? 'Document sans titre'} ${elem.date ? mapToDateHours(elem.date) : 'Date inconnue'}`,
        type: CellType.TEXT
      },
      {
        id: `${elem.id}-ipp`,
