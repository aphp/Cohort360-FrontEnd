...
import { fetchDiagnosticTypes } from 'services/aphp/servicePmsi'
...
const mapConditionFromRequestParams = async (parameters: URLSearchParams) => {
  const code = decodeURIComponent(parameters.get(ConditionParamsKeys.CODE) || '')?.split('|')?.[1] || ''
  ...
  if (diagnosticTypesParams) {
    const allDiagnosticTypes = await fetchDiagnosticTypes()
...
const mapProcedureFromRequestParams = async (parameters: URLSearchParams) => {
  const code = decodeURIComponent(parameters.get(ProcedureParamsKeys.CODE) || '')?.split('|')?.[1] || ''
...
const mapClaimFromRequestParams = async (parameters: URLSearchParams) => {
  const code = decodeURIComponent(parameters.get(ClaimParamsKeys.CODE) || '')?.split('|')?.[1] || ''
...
const mapPrescriptionFromRequestParams = async (parameters: URLSearchParams) => {
  ...
  if (prescriptionTypesParam) {
    const types = await fetchPrescriptions()
...
const mapAdministrationFromRequestParams = async (parameters: URLSearchParams) => {
  ...
  if (administrationRoutesParam) {
    const routes = await fetchAdministrations()
...
const mapBiologyFromRequestParams = async (parameters: URLSearchParams) => {
  const anabioLoinc =
    parameters.get(ObservationParamsKeys.ANABIO_LOINC)?.split('https://terminology.eds.aphp.fr/aphp-itm-') || []
  const anabio = anabioLoinc.find((elem) => elem.includes('anabio|'))?.split('anabio|')?.[1] || ''
  const loinc = anabioLoinc.find((elem) => elem.includes('loinc|'))?.split('loinc|')?.[1] || ''
...
const mapImagingFromRequestParams = async (parameters: URLSearchParams) => {
  ...
  if (modalityParams) {
    const allModalities = await fetchModalities()
...
const mapConditionToRequestParams = (filters: PMSIFilters) => {
  ...
  if (code)
    requestParams += `${ConditionParamsKeys.CODE}=${
      encodeURIComponent('https://terminology.eds.aphp.fr/aphp-orbis-cim10|') + code
    }&`
...
const mapClaimToRequestParams = (filters: PMSIFilters) => {
  ...
  let requestParams = ''
  if (code)
    requestParams += `${ClaimParamsKeys.CODE}=${
      encodeURIComponent('https://terminology.eds.aphp.fr/aphp-orbis-cim10|') + code
    }&`
...
const mapProcedureToRequestParams = (filters: PMSIFilters) => {
  ...
  let requestParams = ''
  if (code)
    requestParams += `${ProcedureParamsKeys.CODE}=${
      encodeURIComponent('https://terminology.eds.aphp.fr/aphp-orbis-cim10|') + code
    }&`
...
const mapBiologyToRequestParams = (filters: BiologyFilters) => {
  ...
  if (anabio || loinc) {
    ...
    if (anabio)
      requestParams += `https://terminology.eds.aphp.fr/aphp-itm-anabio|${anabio.split(',').map((e) => e.trim())}&`
    if (loinc)
      requestParams += `https://terminology.eds.aphp.fr/aphp-itm-loinc|${loinc.split(',').map((e) => e.trim())}&`
  }
