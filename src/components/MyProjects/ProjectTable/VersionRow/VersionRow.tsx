...
const VersionRow: React.FC<{ requestId: string }> = ({ requestId }) => {
  ...
  const [exportedCohort, setExportedCohort] = useState<CohortType | null>(null)
  ...
  const _handleExportCohort = (cohortId: string | null) => {
    const _exportedCohort = cohorts.find(({ uuid }) => uuid === cohortId) ?? null
    setExportedCohort(_exportedCohort)
  }
  ...
  return (
    <>
            ...
            {cohorts && cohorts.length > 0 ? (
              cohorts.map((historyRow) => (
                <TableRow key={historyRow.uuid}>
                  ...
                  <Hidden mdDown>
                    <TableCell align="center">
                      {moment(historyRow.created_at).format('DD/MM/YYYY [à] HH: mm')}
                    </TableCell>
                  </Hidden>
                  <TableCell align="center">
                    <IconButton className={classes.exportButon} onClick={() => _handleExportCohort(historyRow.uuid)}>
                      <ExportIcon />
                    </IconButton>
                  </TableCell>
                </TableRow>
      ...
      <ExportModal open={!!exportedCohort} handleClose={() => _handleExportCohort(null)} cohort={exportedCohort} />
    </>
  )
}
