...
export const constructFhirFilterForType = <T extends SelectedCriteriaType>(
  ...
  return criteriaForm.itemSections
    ...
    .map((item) => {
      ...
      if (buildInfo === undefined || buildInfo.fhirKey === undefined || item.valueKey === undefined) {
        return ''
      }
      const fhirKey = buildInfo.fhirKey
      ...
      return filterValuesArray
        .map((val) =>
          isString(fhirKey) || 'main' in fhirKey
            ? buildFilter(parseFhirKey(fhirKey, deidentified, criteria), val)
            : questionnaireFiltersBuilders(fhirKey, val)
        )
        .join('&')
    })
    .filter((elem) => elem !== '')
    ...
    try {
      ...
      for (const filter of allFilters) {
        ...
        if (key !== null) {
          ...
          if (item && item.buildInfo && item.buildInfo.fhirKey && item.valueKey) {
