...
type CohortCallbacks = {
  ...
  onClickDelete: (cohort: Cohort) => void
...
const CohortsTableContent: React.FC<CohortsTableContentProps> = ({
  ...
  const { onClickRow, onClickFav, onClickExport, onClickEdit, onClickDelete, onSelectCohort } = cohortsCallbacks
  ...
  const columns: Column[] = [
    ...(deleteMode
      ...
      : []),
    ...(!deleteMode ? [{ label: '', code: Order.FAVORITE }] : []),
    ...
    >
      {cohortsList.map((cohort) => {
        ...
        return (
          <TableRow
            key={cohort.uuid}
            onClick={() => (deleteMode ? onSelectCohort(cohort) : onClickRow(cohort))}
              ...
              <TableCellWrapper>
                <Box display={'flex'} alignItems={'center'}>
                  <Tooltip title={getExportTooltip(cohort, !!isExportable)}>
                    ...
                    >
                      <Download fill={disableExport ? '#CBCFCF' : 'black'} />
                    </IconButton>
                  </Tooltip>
                  {/* <Tooltip title={'Créer un échantillon à partir de la cohorte'}>
                      <IconButton
                        ...
                        disabled={disabled}
                      >
                        <Picker stroke={disabled ? '#CBCFCF' : 'black'} />
                      </IconButton>
                    </Tooltip> */}
                  <Tooltip title={'Accéder à la version de la requête ayant créé la cohorte'}>
                    <IconButton
                      ...
                      disabled={disabled}
                    >
                      <RequestTree fill={disabled ? '#CBCFCF' : 'black'} />
                    </IconButton>
                  </Tooltip>
                  <ActionMenu
                    actions={[
